  ...
  this.getAddressSummary(req.addr, options, function (err, data) {
    ...
    if(unspentTxids.length!=0){
      for (let i = 0; i < unspentTxids.length; i++) {
        ...
        this.node.getDetailedTransaction(unspentTxids[i], function (err, transaction) {
          if (err && err.code === -5) {
          ...
          for (let j = 0; j < vouts.length; j++) {
            var rewardData = {
              tiptime: self.node.services.bitcoind.tipTime || moment().unix(), //Using current time if tiptime is not available
        ...
        availableRewardsForAddr = availableRewardsForAddr + availableRewardsForTx;
      }
...
AddressController.prototype.getAddressSummary = function (
  ...
  this.node.getAddressSummary(address, options, function (err, summary) {
    ...
    var transformed = {
      ...
      unspentTxids: summary.unspentTxids,
    };
