    ...
    if (unspentTxids.length != 0) {
            ...
            function (err, transaction) {
              ...
              for (let j = 0; j < vouts.length; j++) {
                ...
                if (vouts[j].address == req.addr || vouts[j].spentTxId) {
                  ...
                  var availableRewardForVout = getKomodoRewards(rewardData);
                }
      ...
      async.parallel(functionsArray, function (err, res) {
        for (let i = 0; i < res.length; i++) {
          availableRewardsForAddr =
            availableRewardsForAddr + res.availableRewardsForTx;
        }
        data.availableRewardsForAddr = availableRewardsForAddr;
      });
    } else {
      data.availableRewardsForAddr = "Not able to Estimate rewards";
    }
    res.jsonp(data);
  });
};
